...
function redirectToWhatsApp(paketId) {
  ...
  const message = `Saya memesan Paket Bandung Utara ${paketId} dengan mobil ${namaMobil} dan berjumlah ${jumlahPenumpang} penumpang.`;
...
document.getElementById("wa-button3").addEventListener("click", function () {
  const paketId = "3";
  const selectElement = document.getElementById(`chooseVehicle${paketId}`);
  const namaMobil =
    selectElement.options[selectElement.selectedIndex].getAttribute(
...
updatePrice("1");
updatePrice("2");
updatePrice("3"); // Memanggil fungsi updatePrice saat tombol diklik
